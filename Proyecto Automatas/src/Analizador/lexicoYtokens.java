...
/** Token Manager. */
public class lexicoYtokens implements lexicoYconstantes
{
...
/** Constructor. */
public lexicoYtokens(SimpleCharStream stream){
   if (input_stream != null)
      throw new tokenerrores("ERROR: Second call to constructor of static lexer. You must use ReInit() to initialize the static variables.", tokenerrores.STATIC_LEXER_ERROR);
   input_stream = stream;
}

/** Constructor. */
public lexicoYtokens(SimpleCharStream stream, int lexState){
   this(stream);
   SwitchTo(lexState);
}
...
static public void SwitchTo(int lexState)
{
   if (lexState >= 1 || lexState < 0)
      throw new tokenerrores("Error: Ignoring invalid lexical state : " + lexState + ". State unchanged.", tokenerrores.INVALID_LEXICAL_STATE);
   else
      curLexState = lexState;
}

static protected tokens jjFillToken()
{
   final tokens t;
   ...
   t = tokens.newToken(jjmatchedKind, curTokenImage);
...
/** Get the next Token. */
public static tokens getNextToken() 
{
  tokens matchedToken;
  int curPos = 0;
  ...
  for (;;)
  {
   ...
   throw new tokenerrores(EOFSeen, curLexState, error_line, error_column, error_after, curChar, tokenerrores.LEXICAL_ERROR);
  }
}
      ...
      case 12 :
         if (jjmatchedPos == -1)
         {
            if (jjbeenHere[0] &&
               ...
               throw new tokenerrores(("Error: Bailing out of infinite loop caused by repeated empty string matches at line " + input_stream.getBeginLine() + ", column " + input_stream.getBeginColumn() + "."), tokenerrores.LOOP_DETECTED);
            jjemptyLineNo[0] = input_stream.getBeginLine();
            jjemptyColNo[0] = input_stream.getBeginColumn();
            jjbeenHere[0] = true;
         }
